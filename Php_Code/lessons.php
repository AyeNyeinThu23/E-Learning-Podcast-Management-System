...
$result = mysqli_query($con, "SELECT * FROM lessons WHERE CID='$cid'");
...
while ($course = mysqli_fetch_assoc($result)) {
    ...
    $filename = basename($course['file_path']);
    $displayName = strpos($filename, '_') !== false ? substr($filename, strpos($filename, '_') + 1) : $filename; 
   
    echo "<tr class='main-row' data-target='details-$cid'>
            <td>" . htmlspecialchars($course['title']) . "</td>
            <td>" . htmlspecialchars($displayName) . "</td>
            <td>" . htmlspecialchars($course['uploaded_at']) . "</td>
            <td>
             <button class='btn btn-primary'><a href='updateLesson.php?ULId=$lessonId'><i class='fas fa-edit'></i></a></button>
             <button class='btn btn-danger'><a href='deleteLesson.php?DLId=$lessonId' onclick=\"return confirm('Are you sure you want to delete this lesson?')\"><i class='fas fa-trash-alt'></i></a></button>
            </td>
            
          </tr>";

    // Hidden details row
    echo "<tr id='details-$cid' class='details-row' style='display:none;'>
            <td colspan='4'>";
    
    // Example: show lessons for this course
    $lessons = mysqli_query($con, "SELECT * FROM lessons WHERE CID='$cid'");
    if(mysqli_num_rows($lessons) > 0){
        echo "<ul>";
        while($lesson = mysqli_fetch_assoc($lessons)){
            echo "<li>" . nl2br($lesson['description'])."</li>";
        }
        echo "</ul>";
    } else {
...
?>
       ...
       <button type="button" class="btn btn-success">
         <a href="Course_Info.php"> Back to Course Info</a>
        </button>

         <button type="button" class="btn btn-success">
        <a href="addLesson.php?cid=<?php echo $cid; ?>">
