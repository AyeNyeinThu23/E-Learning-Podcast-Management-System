...
$result = mysqli_query($con, "SELECT c.CID, c.Title, c.language,c.created_at FROM Courses c JOIN TCA tca ON c.CID = tca.CID WHERE tca.TID = '$tid';");

if (mysqli_num_rows($result) > 0) {
    // ✅ At least one course exists → show table
    ?>
          ...
          <tbody>
           <?php
                    ...
                    while ($course = mysqli_fetch_assoc($result)) {
                        ...
                        $resultForLesson = mysqli_query($con, "SELECT COUNT(*) AS totalLessons FROM lessons WHERE CID = '$cid'");
                        $data = mysqli_fetch_assoc($resultForLesson);

              echo "<tr>";
              echo "<td>" . htmlspecialchars($course["Title"]) . "</td>";
              echo "<td>" . htmlspecialchars($course["language"]) . "</td>";
              echo "<td><a href='lessons.php?id=$cid'>". htmlspecialchars($data['totalLessons']) . "</a></td>";
              echo "<td>" . htmlspecialchars($course["created_at"]) . "</td>";
              echo "<td>
                      <button class='btn btn-primary'><a href='updateCourse.php?upCId=$cid'><i class='fas fa-edit'></i></a></button>
                      <button class='btn btn-danger'><a href='deleteCourse.php?deId=$cid' onclick=\"return confirm('Are you sure you want to delete this course?')\"><i class='fas fa-trash-alt'></i></a></button>
                    </td>";
              echo "</tr>";
           }
                    ?>
    ...
    // ❌ No course → show message instead
    echo "<p style='color: gray; font-style: italic;'>No courses found. Please create a course first.</p>";
}
?>
     <button type="button" class="btn btn-success">
          <a href="addCourse.php"><i class="fas fa-plus"></i> Create Course</a>
        </button>
  </main>
