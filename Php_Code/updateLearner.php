...
<body>
    <a href="userProfile.php" class="back-button">← Back</a>
    <div class="login-container">
        ...
        if (isset($_SESSION['emailforcheckInUpdate'])) {
            ...
            unset($_SESSION['emailforcheckInUpdate']); 
        }
        ?>
        <form method="post" action="updateLearner.php">
             <label>Name:</label>
            ...
            <input type="text" name="password" value="<?php echo htmlspecialchars($_SESSION['real_password']);?>" autocomplete="off">

            <label>Learning Language:</label>
            <input type="text" name="language" id="language" value="<?php echo htmlspecialchars($row['language']);?>" autocomplete="off">
...
if(isset($_POST['update'])) {
    ...
    // Check if email is already used by another learner
    $stmt = $con->prepare("SELECT LID FROM Learner WHERE email = ? AND LID != ?");
    $stmt->bind_param("si", $email, $LID);
    $stmt->execute();
    ...
    $stmt = $con->prepare("UPDATE Learner SET name = ?, email = ?, password = ? WHERE LID = ?");
    $stmt->bind_param("sssi", $name, $email, $encryptedPassword, $LID);
